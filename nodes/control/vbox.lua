...
function VBox:forceRefresh()
	...
	for i = 1, #self.children do
		...
		if child:is(Control) and child._visible then
			...
			containerW = math.max(containerW, childX + childW)
	...
	if resizeToContent then
		...
		chosenOffsetY = math.max(0, math.min(self._offsetY, containerH - selfH))
	end
...
function VBox:_focusOnChild(child)
	if self._allowScrolling then
		...
		self._offsetY = math.max(upperBounds, math.min(self._offsetY, lowerBounds))
		self:deferRefresh()
	end
...
function VBox:_vboxWheelMoved(_, y)
	local newOffset = math.max(0, math.min(self._offsetY - y * self._scrollSpeed, self._calculatedHeight - self._localContentRect.h))
